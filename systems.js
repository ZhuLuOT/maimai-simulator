(function(root){
  ...
  const IDS=['逃遁','鲁米诺','电压','Toqin','我要睡觉','星轨','小盐','404NOTFOUND','八分音符','纸飞机','折返跑','凛冬','橘子汽水','月读','栗子','白昼梦','NekoDX','竹间雨','北纬31','千层雪','再来一把','空白键','mikan','摇光','未完成','七海','镜花','下次一定','微光','晚风'];
  ...
  function ensure(s){
    ...
    const renamed={'凌晨四点':'逃遁','青柠苏打':'鲁米诺','捞月':'电压','阿澈':'Toqin','今天不推分':'我要睡觉'},rename=id=>renamed[id]||id;
  ...
  function grant(s,id){if(has(s,id))return;const t=TALENTS.find(x=>x.id===id);if(!t)throw Error('未知词条。');s.talents.push(id);
    if(['transfer','musician','dragon','foundation','expert'].includes(id))addSkills(s,id==='transfer'?3:id==='dragon'?4:2);
    if(id==='endurance'){s.maxStamina+=10;s.stamina+=10;}if(id==='reader')addSkills(s,4,'reading');if(id==='rich')s.money+=1000;
    if(id==='friends')s.npcs.forEach(n=>n.familiarity=clamp(n.familiarity+25,0,100));if(id==='steady'&&s.condition===0)s.condition=1;
    if(['key','star','slide','reading','speed'].includes(id))addSkills(s,id==='reading'?2:['slide','speed'].includes(id)?4:5,{key:'key',star:'star',slide:'star',reading:'reading',speed:'key'}[id]);
