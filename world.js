(function(root){
  ...
  const QUESTS={
    '电压':{title:'观鸟奇遇',buff:'敏锐观察',effect:'读谱力 +0.25，初见谱面预期达成率 +0.08%',steps:[
      ...
      {name:'翠色的一瞬',place:'yongqing',time:60,stamina:12,cost:0,entry:'kingfisher',text:'蓝色身影一闪，你先发现了水边的翠鸟。电压笑着把这张照片放在图鉴末页：“下次也一起出门吧。”'}]},
  ...
  function install(api){G=api;Object.assign(api,{WORLD_ENTRIES:ENTRIES,QUESTS,worldTick:tick,syncFriends,sendDM:send,questReason,doQuest:quest,startMahjong,finishMahjong,worldScoreBonus:bonus,availableOutings:s=>G.OUTINGS.filter(x=>!LOCATIONS.includes(x.id)||s.world.locations.includes(x.id))});
    const explore=G.explore;G.explore=(s,id)=>{active(s);if(!G.availableOutings(s).some(x=>x.id===id))throw Error('先出门闲逛发现这个地点。');const before={arcades:[...s.city.arcades],restaurants:[...s.city.restaurants]};explore(s,id);afterExplore(s,id,before);};
  }
