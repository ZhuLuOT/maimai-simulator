(function(root){
 ...
 const GOALS=[
  ...
  ...[10000,12000,14000,15000].map((n,i)=>({id:'rating-'+n,name:['迈入万分','稳步向前','摸到高难门槛','最后一千分'][i],text:`达到 ${n.toLocaleString('en-US')} Rating。补齐 B50，复打熟歌，再挑战略高于底力的谱面。`,target:n,value:s=>s.rating,reward:[{skills:{star:.25,key:.25},label:'星星力 +0.25 · 键盘力 +0.25'},{den:true,label:'解锁不眠音游窝 · 24 小时营业 · ¥30 / 小时'},{skills:{star:.3,key:.3,reading:.3},label:'三项底力各 +0.30'},{skills:{star:.35,key:.35,reading:.35},label:'三项底力各 +0.35'}][i],action:'attend'}))
 ];
 function ensure(s){s.playStyle??='balanced';s.guide??={introDone:true,step:0,claimed:[],chatted:s.logs.some(l=>l.text==='在舞萌群聊了 5 分钟。')};s.guide.posted??=0;s.roundReview??=false;if(s.guide.rewardVersion===undefined){for(const id of s.guide.claimed){const g=GOALS.find(g=>g.id===id);if(g)applyReward(s,g.reward,false);}s.guide.rewardVersion=2;}}
 function start(s,style){if(!STYLES[style])throw Error('请选择外键或内屏。');s.playStyle=style;s.skills.star+=STYLES[style].star;s.skills.key+=STYLES[style].key;s.guide={introDone:false,step:0,claimed:[],chatted:false,posted:0,rewardVersion:2};}
 ...
 function postIntro(s){if(s.guide.introDone||s.guide.posted>s.guide.step)return false;s.chat.push(...introMessages(s,s.guide.posted));s.chat=s.chat.slice(-60);s.guide.posted=s.guide.step+1;return true;}
 ...
 function applyReward(s,r,money=true){if(money)s.money+=r.money||0;for(const [skill,amount] of Object.entries(r.skills||{}))s.skills[skill]=Math.min(22,s.skills[skill]+amount);if(r.mood)s.mood=Math.min(100,s.mood+r.mood);if(r.familiarity)s.npcs.forEach(n=>n.familiarity=Math.min(100,n.familiarity+r.familiarity));if(r.den&&!s.city.denUnlocked){s.city.denUnlocked=true;s.chat.push({id:s.npcs[1].id,text:G.homeText(s,'给你发个地址：不眠音游窝，全天开。¥30 / 小时，打歌不另收费，凌晨也能打，记得留钱回家。'),day:s.day,time:s.clock});s.chat=s.chat.slice(-60);}}
