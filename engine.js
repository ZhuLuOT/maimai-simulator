(function(root){
  ...
  function guard(s,phase='home'){if(api.majorBlocked(s))throw Error('请先完成当前挑战或观鸟。');if(s.ending)throw Error('这段故事已经结束。');if(s.school.pending)throw Error('请先回应老师约谈。');if(s.event!==null)throw Error('请先回应小凛。');if(s.world?.notice||s.world?.mahjong.active)throw Error('请先完成当前事件或对局。');if(s.city.encounter)throw Error('请先回应街头偶遇。');if(s.videoEvent)throw Error('请先看完当前手元。');if(s.phase!==phase)throw Error('请先完成当前出勤阶段。');}
  ...
  function rollover(s){
    ...
    if(d.getUTCDate()===1&&j.monthly){const deducted=s.job==='worker'?s.salaryDeduction:0,pay=j.monthly-deducted;s.money+=pay;s.salaryDeduction=0;log(s,`${s.job==='worker'?'工资':'生活费'}到账 +¥${pay}${deducted?`（上月旷工扣薪 ¥${deducted}）`:''}。`,'money');}
  ...
  function expected(s,c){const plays=s.practice[key(c)]||0,difficulty=M.effective(s,c),skill=ability(s,c);
    // 14+ configurations require chart-specific practice as well as fundamentals.
    // Keep official chart constants / Rating intact; ghost charts retain their separate fitted difficulty.
    const advancedLoss=c.ds<14?0:(.35+Math.max(0,c.ds-14)*1.15+Math.max(0,c.ds-14.5)*2)/(1+plays/12)*Math.exp(-Math.max(0,skill-difficulty)*.5);
    const predicted=100.65-J.difficultyPenalty(difficulty,skill)+familiarity(plays)+X.scoreBonus(s,c)+M.score(s,c)+api.foodBonus(s)+api.worldScoreBonus(s,c)-Math.max(0,s.consecutive-3)*.04-advancedLoss;
  ...
  function validate(s){
    ...
    if(typeof s.curseBreakerRewarded!=='boolean'||s.curseBreakerRewarded&&!s.talents?.includes('curse-breaker'))return false;
    if(!MAJOR.valid(s)||!LIN.valid(s)||!SOCIAL.valid(s)||!WORLD.valid(s)||!REST.valid(s)||!GUIDE.valid(s)||!R.valid(s)||!P.valid(s))return false;
    ...
    if(!NEEDS.valid(s)||!num(s.skills.reading,1,22)||!num(s.stamina,0,130)||!num(s.maxStamina,70,130)||!s.gloves||!num(s.gloves.durability,0,240)||!num(s.gloves.wear,.8,1)||!integer(s.condition,0,4)||!num(s.liquid,0,3000)||!integer(s.queueUntil,0,3000)||!Array.isArray(s.talents)||!s.talents.every(id=>api.TALENTS.some(t=>t.id===id))||!Array.isArray(s.npcs)||s.npcs.length!==(30+(s.love>0?1:0))||new Set(s.npcs.map(n=>n.id)).size!==s.npcs.length||s.love>0&&!s.npcs.some(n=>n.id==='小凛')||!s.npcs.every(n=>n&&typeof n.id==='string'&&num(n.rating,0,NPC_RATING_CAP)&&num(n.familiarity,0,100))||!(s.partner===null||integer(s.partner,0,s.npcs.length-1))||!s.profile||typeof s.profile.id!=='string'||typeof s.profile.name!=='string'||!Array.isArray(s.chat)||!s.metrics)return false;
