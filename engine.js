(function(root){
  ...
  const JOBS={student:{name:'学生',money:1800,monthly:1800,daily:35,rent:0,wage:80,workTime:240,icon:'graduation-cap',detail:'月生活费 ¥1800 · 日常 ¥35'},grinder:{name:'挂壁',money:1800,monthly:0,daily:25,rent:600,wage:80,workTime:240,icon:'gamepad-2',detail:'无固定收入 · 月租 ¥600'},worker:{name:'上班族',money:6000,monthly:6000,daily:65,rent:1800,wage:0,workTime:0,icon:'briefcase-business',detail:'月薪 ¥6000 · 月租 ¥1800'}};
  ...
  function daily(s,action){
    ...
    if(action==='work'){if(s.clock<360||s.clock+JOBS[s.job].workTime>NIGHT)throw Error('00:00–06:00 不可打工，兼职也不能跨过午夜。');if(s.job==='worker')throw Error('上班族不能额外打工。');if(s.workCount>=2)throw Error('每天最多接两次兼职。');advance(s,JOBS[s.job].workTime);if(s.ending)return;s.nutrition.away=true;s.workCount++;const income=JOBS[s.job].wage*(s.workCount===1?1:1.5);s.money+=income;s.mood=clamp(s.mood-12,0,100);log(s,`兼职 4 小时，收入 ¥${income}，心情 -12。`,'work');}
  ...
  function expected(s,c){const plays=s.practice[key(c)]||0;return clamp(100.65-J.difficultyPenalty(M.effective(s,c),ability(s,c))+familiarity(plays)+X.scoreBonus(s,c)+M.score(s,c)+api.foodBonus(s)+api.worldScoreBonus(s,c)-Math.max(0,s.consecutive-3)*.04,0,101);}
  ...
  function validate(s){
    ...
    if(!WORLD.valid(s)||!REST.valid(s)||!GUIDE.valid(s)||!R.valid(s)||!P.valid(s))return false;
