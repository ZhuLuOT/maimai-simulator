(function(root){
  ...
  function create(job='student',seed=Date.now()){
    ...
    crowd(s);X.ensure(s);C.ensure(s);CITY.ensure(s);M.ensure(s);R.ensure(s);GUIDE.ensure(s);NEEDS.ensure(s);REST.ensure(s);log(s,'2026 年 3 月 1 日，新学期与新生活开始。','start');return s;
  }
  ...
  function rollover(s){
    ...
    s.day++;s.clock=0;s.nightActive=true;s.completed=[];s.absences=[];s.rests=0;s.workCount=0;s.queueUntil=['play','drink'].includes(s.phase)?Math.max(0,s.queueUntil-NIGHT):0;
  ...
  function daily(s,action){
    ...
    if(action==='work'){if(s.clock<360||s.clock+JOBS[s.job].workTime>NIGHT)throw Error('00:00–06:00 不可打工，兼职也不能跨过午夜。');if(s.job==='worker')throw Error('上班族不能额外打工。');if(s.workCount>=2)throw Error('每天最多接两次兼职。');advance(s,JOBS[s.job].workTime);if(s.ending)return;s.nutrition.away=true;s.workCount++;s.money+=JOBS[s.job].wage;s.mood=clamp(s.mood-12,0,100);log(s,`兼职 4 小时，收入 ¥${JOBS[s.job].wage}，心情 -12。`,'work');}
  ...
  function startTrip(s){guard(s);if(s.clock<OPEN&&s.nightActive&&s.city.denUnlocked&&s.job!=='student')s.arcade=5;s.phase='travel';}
  ...
  function travel(s,id,arcade=s.arcade){
    ...
    if(!den&&(ready+5+r.total>CLOSE))throw Error('机厅 23:30 结束游玩，今天来不及上机了。');if(den&&s.job==='student'&&(s.clock<360||ready+5+r.total+oneWay+35>NIGHT))throw Error('学生需在午夜前回宿舍。');if(s.money<t.cost+entry+r.cost)throw Error(den?'余额不足以支付交通与首小时 ¥30。':`交通费之外还需保留 ¥${r.cost} 上机。`);
  ...
  function availableUntil(s){const den=allNight(s),back=s.trip?.returnTime||0;if(!den&&s.clock>=CLOSE||s.job==='student'&&s.clock<360)return s.clock;const c=nextObligation(s),tomorrow=schedule({...s,day:s.day+1})[0],limit=den?(tomorrow?1440+tomorrow.start-back-35:1440+s.clock):CLOSE;return Math.min(limit,c?c.start-back-35:s.job==='student'?NIGHT-back-35:limit);}
  ...
  function expected(s,c){const plays=s.practice[key(c)]||0;return clamp(100.65-J.difficultyPenalty(M.effective(s,c),ability(s,c))+familiarity(plays)+X.scoreBonus(s,c)+M.score(s,c)+api.foodBonus(s)-Math.max(0,s.consecutive-3)*.04,0,101);}
  ...
  function finishPlay(s){guard(s,'play');s.phase='meal';if(s.trip.rounds&&R.offer(s))s.event=s.love;}
  ...
  function migrate(old){
    if(!old||old.version!==1){if(old?.version===2){X.ensure(old);C.ensure(old);CITY.ensure(old);M.ensure(old);R.ensure(old);GUIDE.ensure(old);NEEDS.ensure(old);REST.ensure(old);}return old;}if(!JOBS[old.job]||!Number.isFinite(old.skill)||!Number.isInteger(old.day)||!old.records)throw Error('旧存档格式错误。');
  ...
  function validate(s){
    ...
    if(!REST.valid(s)||!GUIDE.valid(s)||!R.valid(s)||!P.valid(s))return false;
    ...
    if(s.school.failing&&s.school.since===null)return false;if(!integer(s.love,0,EVENTS.length)||typeof s.loveFailed!=='boolean'||!integer(s.nextLoveVisit,0,1e5)||!(s.event===null||(integer(s.event,0,EVENTS.length-1)&&s.event===s.love&&s.phase==='meal')))return false;
