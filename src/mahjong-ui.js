...
export function yakuCollection(s){const catalog=window.Game.MAHJONG_YAKU,owned=s.world.mahjong.collection;return html`<p>役种收藏 ${Object.keys(owned).length} / ${catalog.length} · 和牌 ${s.world.mahjong.wins} 次</p><div class="yaku-collection">${catalog.map(e=>html`<article class=${owned[e.id]?'collected':''}><div>${icon(owned[e.id]?'check':'lock-keyhole')}<h3>${e.name}</h3><span>${e.han}</span></div><p>${e.description}</p><small>${owned[e.id]?`已和出 ${owned[e.id].count} 次 · 首次第 ${owned[e.id].firstDay} 天`:'尚未和出'}</small></article>`)}</div>`;}
