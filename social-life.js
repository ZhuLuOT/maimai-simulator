(function(root){
 ...
 const JOBS={'电压':'freelance',Toqin:'freelance',COLDDD:'freelance','逃遁':'worker','鲁米诺':'worker','404NOTFOUND':'worker',NekoDX:'student','月读':'freelance'};
 ...
 function voice(s,n,event){const holiday=G.dayInfo(s).holiday||'休息日';
  const special={
   '电压':{holiday:`${holiday}打算先去公园。树上有动静时别急着举相机，先听听。`,free:'刚把今天的鸟类记录整理好。舞萌和观鸟都得慢慢看，急了反而漏细节。',play:'这轮有几段要再看清楚，先歇一下眼睛。',reply:'收到。我先把望远镜收好，晚点看看有没有空一起打。'},
  ...
  const variants={
 '电压':{reply:['在，我刚整理完观鸟记录。你今天有看到什么新动静吗？','我在听今天录到的鸟鸣。打歌的事也可以说。'],play:['刚才那个转折没看清，下轮想先降点难度。','手有点酸了，先喝水，顺便看看今天拍到的鸟。']},
 ...
 function reply(s,id,text){const n=s.npcs.find(x=>x.id===id),c=context(s,n);
  if(/谢谢|多谢|感谢/.test(text))return id==='电压'?'不用谢，下次发现新鸟记得告诉我。':id==='Toqin'?'不客气，能有人一起看这些小细节，我很开心。':id==='COLDDD'?'客气什么，下次继续坐我对面。':'不客气，下次一起打。';
  if(/累|困|睡/.test(text))return id==='Toqin'?'眼睛累的时候我也画不下去。先睡一觉，醒来再看会清楚很多。':id==='COLDDD'?'困了就收，硬撑最容易把安全牌打没。牌桌又不会跑。':id==='电压'?'先去睡吧。精神好一点，鸟鸣和树叶的声音都更容易分清。':'困了就回去休息，别硬推。';
  if(/下班|上班|放假|调休|上课|下课|在哪|在干嘛/.test(text))return voice(s,n,c);
  if(id==='Toqin'){
   if(/观鸟|猫头鹰/.test(text))return '观鸟问电压更靠谱。我会留意它们羽毛的配色，画翅膀时先抓轮廓，再补花纹。';
  ...
  if(/鸟|画/.test(text)&&id!=='电压'&&id!=='Toqin')return '这个得问电压和Toqin，我先把今天这几首练明白。';
