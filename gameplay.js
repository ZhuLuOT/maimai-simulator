(function(root){
  ...
  function staminaCost(s,c){
    const notes=c.notes.reduce((a,b)=>a+b,0),volume=Math.min(notes,700)+Math.max(0,notes-700)*.45;
    const difficulty=Math.pow(Math.max(3,Math.min(c.ds,12))/10,1.7)*(1+Math.max(0,c.ds-12)*.07);
    return (1+volume/180)*difficulty*(s.instinct?1.35:1);
  }
  ...
  function crowdChat(s){
    ...
    const speakers=s.npcs.filter(n=>n.id!=='小凛');if(!speakers.length)return;
    ...
    G.unlockedArcades(s).forEach(({name,id:i})=>{
      ...
      if(high&&!alert.high&&(alert.last<0||now-alert.last>=180)){
        alert.last=now;
        const index=Math.floor(rand(s)*speakers.length),a=speakers[index],b=speakers[(index+1)%speakers.length],c=speakers[(index+2)%speakers.length];
