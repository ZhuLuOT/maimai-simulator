(function(root){
  ...
  function watch(s){if(s.ending||s.phase!=='home'||s.school.pending||s.event!==null||s.videoEvent||s.city.encounter)throw Error('先完成当前事件，再刷视频。');G.advance(s,30);if(s.ending)return;s.mood=clamp(s.mood+7,0,100);const candidates=pool.filter(c=>tag(c)==='ghost'&&!G.isUtage(c)&&Math.abs(c.ds-G.ability(s,c))<=1);if(candidates.length&&rand(s)<.65){const c=candidates[Math.floor(rand(s)*candidates.length)],clear=rand(s)<clamp(.25+(s.skills.reading-c.ds)*.08,.12,.75);s.videoEvent={key:G.key(c),outcome:clear?'clear':'partial'};G.log(s,`刷到 ${c.title} ${G.displayLevel(c)} 的手元，停下来研究了一会儿。`,'event');}else G.log(s,'刷视频 30 分钟，心情 +7。','heart');G.check(s);}
  ...
  function snapshot(s){const b=G.best(s),compact=r=>({id:r.id,index:r.index,title:r.title,type:r.type,ds:r.ds,achievement:r.achievement,ra:r.ra,combo:r.bestCombo||r.combo||'',isNew:r.isNew});return {name:s.profile.id,plate:s.profile.plate,title:s.profile.title,avatar:s.profile.avatar,rating:s.rating,day:s.day,old:b.old.map(compact),fresh:b.fresh.map(compact)};}
