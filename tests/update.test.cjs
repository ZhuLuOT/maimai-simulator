...
test('NPC preferences persist, choose some challenging charts and celebrate actual simulated scores',()=>{const s=arrive(),n=s.npcs[0];n.rating=14000;n.genre=pool.find(c=>c.ds===13).genre;n.tendency='star';const picks=M.choose(n,s,300);assert.ok(picks.some(c=>c.ds>n.rating/1110+.5));assert.ok(picks.some(c=>c.genre===n.genre));for(let i=0;i<10;i++)M.npcTick(s,20);assert.ok(s.chat.some(m=>m.text.includes('今天推上去了')));const boast=s.chat.findIndex(m=>m.performance);assert.ok(boast>=0);const reply=s.chat[boast+1];if(reply)assert.ok(reply.id!==s.chat[boast].id&&reply.text);for(const m of s.chat.filter(m=>!m.self)){const previous=s.chat.slice(0,s.chat.indexOf(m)).findLast(x=>x.id===m.id);if(previous)assert.ok((m.day-previous.day)*1440+m.time-previous.time>=20);}const restored=G.migrate(JSON.parse(JSON.stringify(s)));assert.equal(restored.npcs[0].genre,n.genre);assert.ok(G.validate(restored));});
