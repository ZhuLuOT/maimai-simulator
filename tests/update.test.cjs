...
test('NPC preferences persist, choose some challenging charts and celebrate actual simulated scores',()=>{const s=arrive(),n=s.npcs[0];n.rating=14000;n.genre=pool.find(c=>c.ds===13).genre;n.tendency='star';const picks=M.choose(n,s,300);assert.ok(picks.some(c=>c.ds>n.rating/1110+.5));assert.ok(picks.some(c=>c.genre===n.genre));for(let i=0;i<10;i++)M.npcTick(s,20);assert.ok(s.chat.some(m=>m.text.includes('今天推上去了')));const boast=s.chat.findIndex(m=>m.performance);assert.ok(boast>=0);const reply=s.chat[boast+1];assert.ok(reply&&reply.id!==s.chat[boast].id&&reply.text&&!reply.performance);const restored=G.migrate(JSON.parse(JSON.stringify(s)));assert.equal(restored.npcs[0].genre,n.genre);assert.ok(G.validate(restored));});
