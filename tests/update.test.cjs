...
test('bot help, live crowd counts and B50 snapshot cost no time or familiarity; messages survive reload',()=>{const s=arrive();G.play(s,G.recommend(s,pool),pool);G.finishPlay(s);G.meal(s,'home');const clock=s.clock,familiarity=s.npcs[0].familiarity;G.chatSend(s,'@bot');assert.match(s.chat.at(-1).text,/快捷指令/);G.chatSend(s,'jk');assert.ok(s.chat.at(-1).text.includes(`${G.peopleAt(s,0)} 人`));G.chatSend(s,'几卡');G.chatSend(s,'B50');const snap=s.chat.at(-1).b50;assert.equal(snap.old.length+snap.fresh.length,3);assert.equal(snap.rating,s.rating);assert.equal(s.clock,clock);assert.equal(s.npcs[0].familiarity,familiarity);s.rating++;assert.notEqual(snap.rating,s.rating);assert.ok(G.validate(G.migrate(JSON.parse(JSON.stringify(s)))));});
...
test('crowded arcades trigger discussion and hesitation once per rising threshold with cooldown and save persistence',()=>{
  ...
  s.people=7;M.crowdChat(s);assert.equal(s.chat.length,3);assert.match(s.chat[0].text,/次元空间现在 11 人，大B队来了/);
