...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.locator('[data-action="quest"][data-value="电压"]').click();assert.match(await page.locator('.world-event-text').innerText(),/领角鸮/);assert.match(await page.locator('.world-event-text').innerText(),/解锁新地点：白云山/);
