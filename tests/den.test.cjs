...
function unlocked(job='grinder'){const s=G.create(job,42);s.rating=12000;G.claimGoal(s,'rating-12000');s.rating=0;s.money=10000;return s;}
...
test('new rewards grant skill, mood and familiarity with caps; legacy claims receive upgrades once',()=>{
 ...
 const old=G.create();old.guide.claimed=['first-pc','first-s','rating-12000'];delete old.guide.rewardVersion;const cash=old.money,key=old.skills.key;G.migrate(old);assert.equal(old.money,cash);assert.equal(old.skills.key,key+.15);assert.equal(old.city.denUnlocked,true);const snapshot=JSON.stringify(old);G.migrate(old);assert.equal(JSON.stringify(old),snapshot);assert.ok(G.validate(old));
});
test('den requires a goal unlock and is separate from the five discoverable arcades',()=>{
