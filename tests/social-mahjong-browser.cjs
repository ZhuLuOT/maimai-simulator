...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await live.goto('http://127.0.0.1:4173/');await live.locator('.release-dialog[open]').waitFor();assert.deepEqual(await live.locator('.release-changes li').allTextContents(),require('../src/release-notes.cjs').changes.slice(0,8));assert.doesNotMatch(await live.locator('.release-dialog').innerText(),/赛季|6月30日/);await live.screenshot({path:path.resolve(__dirname,'../artifacts/revision-release.png')});
