...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await live.goto('http://127.0.0.1:4173/');await live.locator('.release-dialog[open]').waitFor();assert.equal(await live.locator('.release-changes li').count(),5);assert.doesNotMatch(await live.locator('.release-dialog').innerText(),/赛季|6月30日/);await live.screenshot({path:path.resolve(__dirname,'../artifacts/revision-release.png')});
