...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.locator('[data-action="chat"]').click();await page.locator('[data-action="conversation"][data-value="小凛"]').click();await page.locator('[data-action="relationship"]').click();assert.match(await page.locator('.relationship-summary').innerText(),/故事 2 \/ 8/);assert.match(await page.locator('.relationship-memories').innerText(),/小凛/);
 await page.locator('[data-action="love-contact"][data-value="chat"]').click();assert.match(await page.locator('.relationship-summary').innerText(),/21\/100/);assert.ok(await page.locator('[data-action="love-contact"][data-value="walk"]').isDisabled());await shot('relationship');await close();await close();assert.equal(await page.locator('.clock-display>b').innerText(),'08:15');
