...
test('daytime revisits can collect all twenty additional birds without consuming quest unlocks',()=>{
 ...
 for(const place of s.world.locations){
  ...
  for(let i=0;i<birds.length;i++){
   ...
   const count=s.world.entries.length;G.explore(s,place);
   assert.equal(s.world.entries.length,count+1);assert.ok(birds.some(e=>e.id===s.world.notice.entry));assert.equal(s.money,992);assert.ok(G.validate(s));
  }
...
test('bird discoveries require the first quest and a daytime visit',()=>{
 ...
 const s=G.create('grinder',42);s.world.locations=['yuexiu'];s.world.quests['电压'].stage=1;s.clock=360;s.stamina=100;G.explore(s,'yuexiu');assert.equal(s.world.entries.length,1);
});
...
test('final bird quest enforces the night window, crosses midnight, and migrates old rewards once',()=>{
 ...
 for(const clock of [0,150,1320,1410]){const s=ready();s.clock=clock;const day=s.day;G.doQuest(s,'电压');assert.equal(s.world.quests['电压'].stage,5);assert.equal(s.world.notice.entry,'owl');assert.ok(G.availableOutings(s).some(x=>x.id==='baiyun'));assert.equal(s.day,day+(clock===1410?1:0));assert.ok(G.validate(s));}
 const old=ready();delete old.world.quests['电压'].rewarded;old.skills.reading=8.25;G.migrate(old);assert.equal(old.world.quests['电压'].rewarded,true);assert.ok(G.worldScoreBonus(old,{id:'1',index:0})>.07);old.clock=1320;G.doQuest(old,'电压');assert.equal(old.skills.reading,8.25);G.migrate(old);assert.equal(old.skills.reading,8.25);
