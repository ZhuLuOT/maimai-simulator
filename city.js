(function(root){
  ...
  const RESTAURANTS=[
    {id:'rice-roll',name:'西关肠粉小店',cost:16,time:15,mood:22,stamina:45,buff:.06,icon:'utensils',note:'心情 +22 · 体力 +45 · 饱足 +0.06'},
    {id:'wonton',name:'竹升云吞面馆',cost:24,time:20,mood:25,stamina:50,buff:.08,icon:'soup',note:'心情 +25 · 体力 +50 · 饱足 +0.08'},
    {id:'claypot',name:'老街煲仔饭',cost:32,time:25,mood:30,stamina:60,buff:.1,icon:'cooking-pot',note:'心情 +30 · 体力 +60 · 饱足 +0.10'}
  ];
  const TAROT=['愚者','魔术师','女祭司','皇后','皇帝','教皇','恋人','战车','力量','隐者','命运之轮','正义','倒吊人','死神','节制','恶魔','高塔','星星','月亮','太阳','审判','世界'];
  function ensure(s){s.city??={arcades:[0,1,2],restaurants:[],visited:[],encounter:false,walks:0,foodUntil:0,foodBuff:0};s.city.denUnlocked??=false;s.botUsage??={day:s.day,b50:0,crowd:0,fortune:null,tarot:0};s.competition??={wins:0,losses:0,draws:0,battle:false,courses:[],last:null};}
  ...
  function explore(s,id){
    ...
    if(x.stamina){
      ...
      else if(roll<.45&&s.city.arcades.length<5){const a=ARCADES.find(a=>!a.special&&!s.city.arcades.includes(a.id));s.city.arcades.push(a.id);G.log(s,`发现新机厅：${a.name}！已加入机厅地图。`,'event');say(s,s.profile.id,`散步发现了${a.name}，有 ${a.cabinets} 台机组！`);say(s,s.npcs[2].id,'新店！下次一起去探探。');say(s,s.npcs[7].id,'我先记下了，人少就去出勤。');}
  ...
  function valid(s){const c=s.city,u=s.botUsage,p=s.competition,int=(x,max)=>Number.isInteger(x)&&x>=0&&x<=max;
    ...
    return !!c&&Array.isArray(c.arcades)&&[0,1,2].every(i=>c.arcades.includes(i))&&new Set(c.arcades).size===c.arcades.length&&typeof c.denUnlocked==='boolean'&&c.arcades.length<=5&&c.arcades.every(i=>int(i,4))&&Array.isArray(c.restaurants)&&c.restaurants.length<=3&&c.restaurants.every(id=>RESTAURANTS.some(r=>r.id===id))&&Array.isArray(c.visited)&&c.visited.length<=5&&c.visited.every(id=>OUTINGS.some(x=>x.id===id&&x.stamina))&&typeof c.encounter==='boolean'&&int(c.walks,20000)&&Number.isFinite(c.foodBuff)&&c.foodBuff>=0&&c.foodBuff<=.1&&int(c.foodUntil,(G.DAYS+1)*1440)&&!!u&&int(u.day,G.DAYS)&&u.day>=1&&['b50','crowd','tarot'].every(k=>int(u[k],1e5))&&(u.fortune===null||int(u.fortune,100))&&!!p&&['wins','losses','draws'].every(k=>int(p[k],1e5))&&typeof p.battle==='boolean'&&Array.isArray(p.courses)&&new Set(p.courses).size===p.courses.length&&p.courses.length<=10&&p.courses.every(n=>int(n,10)&&n>=1);
  }
  function install(api){G=api;Object.assign(api,{ARCADE_LIMIT:5,ARCADES,OUTINGS,RESTAURANTS,unlockedArcades:s=>ARCADES.filter(a=>a.special?s.city.denUnlocked:s.city.arcades.includes(a.id)),explore,answerEncounter:encounter,mealOptions:meals,applyFoodBuff:food,foodBonus:s=>now(s)<s.city.foodUntil?s.city.foodBuff:0});}
