import {createSongSearch} from './src/song-search.cjs';
...
import {startTable,chooseMahjong,tapMahjong,autoMahjong,abandonMahjong,mahjongView,yakuCollection} from './src/mahjong-ui.js';
import {linPanel} from './src/lin-ui.js';
import {installUpdateNotice} from './src/update-notice.js';
import {calendar as calendarView,timetable as timetableView} from './src/calendar-ui.js';
(() => {
  'use strict';
  ...
  function render() {
    ...
    mount(html`<header class="topbar"><a class="brand" href="#" data-action="nav" data-value="home"><span class="brand-mark">${icon('disc-3')}</span><span>出勤模拟器<span class="brand-en">MAIMAI LIFE SIMULATOR</span></span></a><nav aria-label="主导航">${[['home', 'house', '日常'], ['library', 'disc-3', '曲库'], ['best', 'trophy', 'B50'], ['journal', 'notebook-pen', '手账']].map(([v, i, n]) => html`<button class="nav-item ${v === view ? 'active' : ''}" aria-label="${n}" title="${n}" data-action="nav" data-value="${v}">${icon(i)}<span>${n}</span></button>`)}</nav><div class="top-tools"><span class="save-state"><span></span>自动存档</span><button class="icon-btn" data-action="sound" title="${sound ? '关闭' : '开启'}音效" aria-label="音效">${icon(sound ? 'volume-2' : 'volume-x')}</button><button class="icon-btn" data-action="settings" title="存档与设置" aria-label="存档与设置">${icon('settings-2')}</button></div></header>
      <main><div class="date-toolbar"><button class="calendar-date" data-action="calendar" title="查看日历" aria-label="查看日历">${icon('calendar-days')}<b>${G.dateLabel(state)}</b><span>${G.dayInfo(state).holiday||G.dayInfo(state).makeup||`第 ${state.day} / ${G.DAYS} 天`}</span></button><div class="clock-display">${icon('clock-3')}<b>${G.time(state.clock)}</b><span>${next ? `${next.name} ${G.time(next.start)}` : '今日无剩余日程'}</span></div></div>
      ${LifeUI.player(state)}
  ...
  function filtered() {
    ...
    return songs.filter(s => (typeFilter === 'all' || s.type === typeFilter) && (eraFilter === 'all' || s.version === eraFilter) && (genreFilter === 'all' || s.genre === genreFilter) && (utageFilter === 'all' || utageFilter === 'only' === G.isUtage(s)) && (!q || /^\d{1,2}\+?$/.test(q) || matchesSong(s,q)) && s.ds.some((_, i) => {
  ...
  function bestView() {
    const b = G.best(state),
      toggle = html`<div class="section-heading"><h2 class="best-title">B50 <small class="game-version">出勤模拟器 · ${__APP_RELEASE__}</small></h2><div class="view-toggle"><button class="secondary-btn" data-action="best-style" data-value="list" aria-pressed="${bestStyle === 'list'}">${icon('list')}列表</button><button class="secondary-btn" data-action="best-style" data-value="image" aria-pressed="${bestStyle === 'image'}">${icon('image')}成绩图</button></div></div>`;
  ...
  function options(list, action) {
    return html`<div class="option-grid">${list.map(x => {
      const unavailable = x.cost > state.money || action==='eat-home'&&(!G.canEatHome(state,x.id)) || action === 'meal' && (mealDestination==='arcade'?!!G.returnToPlayReason(state,x.id):(!G.canSpendTime(state, x.time + state.trip.returnTime)));
  ...
  function renderModal() {
    ...
    if(modal==='intro')content=frame('出发之前','和逃遁、鲁米诺聊聊',intro(state,introReplay),false,false);
    if(modal==='calendar')content=frame('春季日历','2026 年 3 月至 6 月',calendarView(state,calendarMonth,calendarDay));
    if (modal === 'chat') content=frame('手机','群聊、好友与支线',chatApp(state,conversation,conversationList),true);
    ...
    if(modal==='mahjong-collection')content=frame('和牌役种','猫窝 · 日式麻将',yakuCollection(state),true);
    if(modal==='mahjong')content=frame('猫窝麻将','立直麻将一局战 · '+(state.world.mahjong.active?.players||[]).join(' / '),mahjongView(),true,false);
    ...
    if (modal === 'relationship') content = frame('与小凛的日常',G.relationshipLabel(state),linPanel(state,true));
    ...
    if (modal === 'timetable') content = frame('本周课表','2026 春季',timetableView(state),true);
    if (modal === 'event') {
    ...
    if (modal === 'about') content = frame('规则与来源', '2026 SPRING', html`<div class="about-copy"><h3>2026 年 3 月 1 日至 6 月 30 日</h3><p>共 122 天，每天 24 小时。睡眠可自选 0.5–12 小时，一次睡到爽为 10 小时，行动消耗分钟；机厅 10:00 开门、23:30 结束游玩。吃饭、回程均耗时，午夜结算当天收支。按 2026 年法定节假日及调休安排日程：上班日 09:00–18:00，学生按课表上课，5 月 9 日补周二课表。点击日期可查看日历。</p><h3>经济与学业</h3><p>开局资金：学生 ¥1800、挂壁 ¥1800、上班族 ¥6000。每月 1 日学生生活费 ¥1800、上班族工资按 ¥6000 减去上月旷工扣薪入账，开局已含 3 月资金。旷工每次记扣薪 ¥273，仅从当月工资扣除，累计不超过月薪，不立即扣钱包现金。每日基础开销分别为 ¥35 / ¥25 / ¥65。挂壁房租 ¥600，上班族 ¥1800，25 日零点检查并缴纳。上班族不可打工。</p><p>水课被抓概率 35%，专业课 75%。学力归零进入挂科并触发约谈；恢复到 20 解除。第三次约谈或连续挂科 10 天进入肄业结局。</p><h3>分数与底力</h3><p>达成率上限 101.0000%，Rating 仅计算到 100.5%。旧版本最佳 35 张 + 新版本最佳 15 张。同谱面初见有 -0.55 个百分点的预期修正，重复游玩逐步增加熟练度；实际成绩还受底力、心情、饮品、疲劳与判定波动影响。</p><p>FC：没有 MISS；FC+：没有 GOOD / MISS；AP：没有 GREAT / GOOD / MISS。成绩与徽章由模拟判定统计产生，不是通过百分比直接指定。判定分布是本作模拟，非官方谱面重放。</p><p>星星 / 键盘优先采用 ChiffonMai 同源 DXRating 社区标签，未标注谱面按音符占比估算。海底谭 MASTER 保留人工校正。对应底力影响达成率，也会获得更多成长。单开三首，拼机通常双方各选两首、同步游玩四首；玩家可单独选择拼机伙伴选曲的难度。两人一组计算排队，普通机厅每人 ¥6；猫窝每人 ¥30 / 小时，预付首小时，续时按整小时收费，PC 和段位不另计费。</p><h3>午夜与宴曲</h3><p>午夜自动跨日并结算生活开销、工资和房租。各职业均可凌晨外出或返回，仍需避开实际课程与班次。清醒行动每小时增加 6.25 点困意，连续清醒 16 小时到满，超过 60 开始影响成绩，达到 100 终止行动并回家休息 8 小时；睡眠覆盖课程或班次记为旷课、旷工。00:00–06:00 不能打工。</p><p>名称以方括号标签开头的曲目统一作为宴曲：不自动推荐、不参与 B50，手动游玩保留达成率但 Rating 为 0。</p><h3>三餐与饮料</h3><p>早餐 06:00–11:00、午餐 11:00–17:00、晚餐 17:00–24:00 各记一次；在住处或准备出门时，08:00 / 12:00 / 18:00 后首次操作提醒选饭。上班、长课程中途可午休吃饭。学生回宿舍，可选美团拼好饭 ¥12。基础餐食包含在每日开销中；萨莉亚 ¥25 / 心情 +18，KFC 平日 ¥50、周四 ¥29.9。</p><p>连续漏餐 3 天后，每个漏餐日最大体力 -2，下限 70；连续 3 天吃齐三餐并出门活动，最大体力 +1，上限 120（体力过人为 130）。最多带 3 瓶饮料，大水最多 2 瓶：大水 1L / ¥5，乌龙茶 500ml / ¥5，冰美式 200ml / ¥9.9，魔爪 300ml / ¥10；认识小凛后解锁粉色魔爪 300ml / ¥12，状态与心情加成更高。可以切换正在喝的饮料，喝完自动开下一瓶，剩余饮料会随身保留。</p><h3>体力与成长</h3><p>手套每曲磨损，耐久不足一轮时须更换。每曲消耗体力与 60 ml 饮料；体力低于 55 逐渐减分，饮料耗尽额外减分。难度与物量越高消耗越快。等待每分钟恢复 0.35，吃饭恢复 30–60；家或宿舍内可自选睡眠时长，每小时消除 12.5 困意并恢复体力上限的 1/8；普通娱乐不回体。非饭点且本次消耗较少可直接返回住处。状态每日结合心情随机决定；读谱跟不上时，高难发挥会受限。饮料、好心情与临时词条的正向底力增益合计最多 +0.75。熟悉谱面和长期准度训练有助于稳定高分，二见不等于掌握；即使 AP，BREAK 不够准也无法达到理论值。鬼歌以拟合定数计算表现、初见额外 -0.45%，成长 ×1.3；吃分预期 +0.16%，成长 ×0.7。刷视频可能学到鬼歌手元：大彻大悟降低该谱面计算难度，似懂非懂免除额外初见惩罚。词条依据本地 maimai-talent-tags.md。</p><p>推荐会搭配适合当前底力与稍高难度的曲目，“换一组”避开当前歌曲。定数略高于分类底力的谱面获得更多成长，差距过大则降低成长效率。底力越高，每次练习的增长越少，提升同样幅度需要更多时间。越级按游玩前 Rating、官方标级与成绩不高于 97% 判断：低于 11,000 对应 11+，11,000 对应 12，12,000 对应 13+，13,000 对应 14，14,000 及以上对应 14+。</p><h3>群聊与广州闲逛</h3><p>群聊输入 @bot 查看快捷指令；运势每天固定，塔罗每次 2–3 分钟。B50 与人数查询每天各首次免费，之后每次 2 分钟。闲逛及游览广州景点消耗体力、恢复心情，可能遇到小凛、发现新机厅或餐馆；发现的店铺会永久加入选项。</p><h3>名牌与拟合</h3><p>版本名牌参考公开查分器规则：极为全 BASIC–MASTER FC，将为全 SSS，神为全 AP。曲目范围与难度要求按落雪收藏品 API 核验；舞系包含指定 Re:MASTER。真系没有“真将”。收录 71 块极 / 将 / 神名牌，另有签到、区域装饰和称号。区域装饰以选定区域累计 10 PC 解锁，每日首次上机后自动签到，增加 2 点进度；已移除要求觉醒的称号；移动距离称号按实际往返机厅的公里数累计。舞舞牌按同步最佳成绩逐谱面解锁。收藏与上机页可进入模拟段位及友人对战。</p><p>拟合数据来自水鱼 chart_stats，落雪公开曲库用于交叉核对官方定数。样本至少 100、两站定数一致且拟合差值绝对值 ≥0.3 才打标签。未获得落雪公开拟合数据，因此不是两站拟合共识。数据快照 2026-09-14。</p><h3>素材与数据</h3><p>非官方同人游戏。${songs.length} 首曲目与定数来自 <a href="https://www.diving-fish.com/api/maimaidxprober/music_data" target="_blank" rel="noreferrer">水鱼公开曲库</a>（2026-09-14 快照）。版本按落雪国服曲库映射为舞萌DX 2020–2026，B50 按国服引入版本分组，与故事月份独立。</p><p>曲绘来自水鱼 covers 公开图片服务，名牌与分数框来自 <a href="https://github.com/Yuri-YuzuChaN/maimaiDX" target="_blank" rel="noreferrer">maimaiDX</a> 的 CN1.55 公开素材包，本作现用国服“舞萌 DX”标识适配框，非原始官方框截图；扩充的名牌图案与称号条件来自落雪公共服务；社区技术标签来源经 <a href="https://github.com/ChiffonOwO/ChiffonMai" target="_blank" rel="noreferrer">ChiffonMai</a> 核对。相关版权归原权利人。无官方音源。图标使用 Lucide。</p></div>`);
  ...
  function act(a, v) {
    ...
    switch (a) {
      ...
      case 'intro-replay':state.guide.step=0;introReplay=true;modal='intro';break;
      case 'calendar':calendarDay=state.day;calendarMonth=G.date(state).getUTCMonth()+1;modal='calendar';break;
      case 'calendar-today':calendarDay=state.day;calendarMonth=G.date(state).getUTCMonth()+1;break;
      case 'calendar-month':calendarMonth=Math.max(3,Math.min(6,calendarMonth+Number(v)));calendarDay=Math.round((Date.UTC(2026,calendarMonth-1,1)-G.START)/86400000)+1;break;
      case 'calendar-day':calendarDay=Number(v);break;
      ...
      case 'original-avatar':
        ...
        break;
      case 'lin-story':G.openLinStory(state);modal='event';break;
      case 'lin-pair':G.pairWithLin(state);recommend();modal='trip';break;
      case 'love-contact':
      ...
      case 'mahjong':
        if(!G.startMahjong(state))break;
        startTable(state.profile.name,()=>renderModal(),result=>{
          G.finishMahjong(state,result);
          abandonMahjong();
          modal=state.world.notice?'world-event':'trip';
          save();
        },{players:state.world.mahjong.active.players,tutorial:!state.world.mahjong.tutorialDone&&state.world.mahjong.active.players.includes('COLDDD')});
        modal='mahjong';break;
      case 'mahjong-tile':tapMahjong(Number(v));break;
      case 'mahjong-choice':chooseMahjong(Number(v));break;
      case 'mahjong-help':autoMahjong();break;
      case 'mahjong-collection':mahjongCollectionReturn=modal;modal='mahjong-collection';break;
      ...
      case 'finish':
        ...
        break;
      case 'answer': {
        const first=state.event===0;
        G.answer(state, Number(v));
        if(first)modal='trip';else{conversation='小凛';conversationList=false;modal='chat';}
        break;
      }
